...
| start_at | datetime (UTC) | Начало в UTC; отображение — локальная зона; отмена ≥2 ч — сервер (R-021) |
| total_seats | int | Всего мест (≤ program.capacity_cap) |
| free_seats | int | Свободно мест |
| free_rental_kits | int | Свободно прокатных комплектов (фонд мастерской до **8**) |
...
| `active` | cancel ≥2 ч | `cancelled` | Места и прокат **возвращаются** |
| `active` | cancel <2 ч | `late_cancel` | Места и прокат **не освобождаются** |
...
- `max_seats_for_booking = min(free_seats, program.capacity_cap, 3)` (FR-13).
